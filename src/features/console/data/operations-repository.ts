...
export async function listTeamMembers(context: BusinessContext): Promise<TeamMember[]> { const supabase = await createClient(); const { data, error } = await supabase.from("profiles").select("id, display_name, email, role").eq("business_id", context.businessId).eq("is_active", true).order("display_name"); if (error) throw new Error(error.message); return z.array(profileRowSchema).parse(data || []).map((row) => ({ id: row.id, name: row.display_name || row.email || "Team member", email: row.email || "", role: row.role === "co_owner" ? "Co-owner" : row.role === "technician" ? "Technician" : "Owner" })); }
